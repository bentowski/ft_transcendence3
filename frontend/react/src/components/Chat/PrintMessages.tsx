...
class DispatchMsg extends Component<{user: UserType, messages: any}, {userList: UserType[]}> {
	...
	// updateUsers = async () => {
	// 	let users:UserType = await Request("GET", {}, {}, "http://bentowski.fr:3000/user")
      ...
      if (msg.sender_socket_id === this.props.user.auth_id)
        ...
        ret.push(
            <div key={index} className="incoming_msg break-text">
              <div className="incoming_msg_img align-bottom"> <img src={"http://bentowski.fr:3000/user/" + msg.auth_id + "/avatar"} alt="ImageNotFound" /> </div>
