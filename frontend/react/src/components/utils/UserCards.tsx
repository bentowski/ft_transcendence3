...
> {
  ...
  createChan = async (): Promise<string> => {
    ...
    try {
      ...
      if (newChan !== undefined) {
        ...
        //window.location.href = "http://192.168.0.1/chat/" + newChan.id
    ...
    if (x === chans.length) {
      ...
      let newUrl = "http://192.168.0.1/chat/#" + newChan.id;
      setTimeout(() => {
        window.location.href = newUrl;
      }, 100);
      return;
    }
    let newUrl = "http://192.168.0.1/chat/#" + ret;
    window.location.href = newUrl;
     */
  };
  ...
  startNewGame = async (): Promise<void> => {
    ...
    //   return p.id;
    // })
    // window.location.href = "http://192.168.0.1/game/" + Math.max(...ids)
  }
