...
> {
  ...
  //                             <img alt="" src={'http://bentowski.fr:3000/user/' + usersChan[x].user?.auth_id + '/avatar'} className="miniAvatar" width={150} height={150}/>
  //                         </div>
  //                     </div>
  //                 );
  //             }
  ...
  joinPrivateChan = async (): Promise<void> => {
    ...
    try {
      const chan: ChanType = await Request(
          ...
          {},
          {},
          "http://bentowski.fr:3000/chan/" + input.value
      );
