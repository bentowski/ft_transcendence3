...
class ModalMatch extends Component<{ title: string, calledBy: string }, {}> {
  ...
  createParties = async (isClassic: number): Promise<void> => {
    ...
    try {
      ...
      this.hidden()
      if (isClassic)
        window.location.href = "http://localhost:80/game/" + Math.max(...ids)//currentUser.user.username
      else
        window.location.href = "http://localhost:80/gameup/" + Math.max(...ids)//currentUser.user.username
      } catch (error) {
       const ctx: any = this.context;
       ctx.setError(error);
    }
